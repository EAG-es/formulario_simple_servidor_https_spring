...
import inweb.spring.formulario_simple_servidor_https_spring.controlador.Pagina_inicio;
...
public class Formulario_simple_servidor_https_spring extends Servidor_https_spring {

    public static void main(String[] args) {
        oks ok = new oks();
        try {
            Formulario_simple_servidor_https_spring servidor_https_spring 
                    = new Formulario_simple_servidor_https_spring();
            servidor_https_spring.run(ok, Formulario_simple_servidor_https_spring.class, args);
        } catch (Exception e) {
            ok.setTxt(e);
        }
        if (ok.es == false) {
    ...
    public boolean iniciar(oks ok, Object... extra_array) throws Exception {
        ...
        if (ok.getGravedad() > oks.k_gravedad_baja) { return ok.es; }
        ok.iniciar();
        _iniciar_desde_clase(Formulario_simple_servidor_https_spring.class, ok);
        if (ok.getGravedad() > oks.k_gravedad_baja) { return ok.es; }
        ok.iniciar();
        return ok.es;
    }
    ...
    public boolean terminar(oks ok, Object... extra_array) throws Exception {
        ...
        if (ok.getGravedad() > oks.k_gravedad_baja) { return ok.es; }
        ok.iniciar();
        _terminar_desde_clase(Formulario_simple_servidor_https_spring.class, ok);
        if (ok.getGravedad() > oks.k_gravedad_baja) { return ok.es; }
        ok.iniciar();
        return ok.es;
    }
